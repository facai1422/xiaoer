...
// USDT图标组件
const USDTIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 32 32" className="inline-block mr-1">
...
export const BalanceStats = ({ balanceStats, onNavigate }: BalanceStatsProps) => {
  return (
    <div className="grid grid-cols-2 gap-4 mx-5">
      <div className="bg-white rounded-xl p-4">
        <div className="text-xl font-bold text-green-500 mb-2 flex items-center">
          <USDTIcon />
          {balanceStats.availableBalance.toFixed(2)}
        </div>
        <div className="flex items-center justify-between">
          <div className="text-gray-600 text-sm">可用余额</div>
          <button onClick={() => onNavigate('/wallet-recharge')} className="px-4 py-1 bg-green-500 text-white rounded-full text-sm">
            充值
          </button>
        </div>
      </div>
      <div className="bg-white rounded-xl p-4">
        <div className="text-xl font-bold text-orange-500 mb-2 flex items-center">
          <USDTIcon />
          {balanceStats.frozenBalance.toFixed(2)}
        </div>
        <div className="flex items-center justify-between">
          <div className="text-gray-600 text-sm">冻结余额</div>
          <button onClick={() => onNavigate('/wallet')} className="px-4 py-1 bg-orange-500 text-white rounded-full text-sm">
            详情
          </button>
        </div>
      </div>
